...
class _RestaurantMenuScreenState extends State<RestaurantMenuScreen>
  ...
  double get _cartTotal => calculateLocalCartTotal(_cart);

  int get _cartItemCount => calculateLocalCartItemCount(_cart);
  ...
  // ═══════════════════════════════════════════════════════════════
  // ORDER PLACEMENT - FIXED WITH MOUNTED CHECKS
  // ═══════════════════════════════════════════════════════════════
  ...
  // ═══════════════════════════════════════════════════════════════
  // PAYMENT HANDLERS - FIXED WITH MOUNTED CHECKS
  // ═══════════════════════════════════════════════════════════════

  Future<void> _payWithPayChangu(String orderId, double amount) async {
    Navigator.pop(context);

    // FIXED: Check mounted after popping payment sheet
    if (!mounted) return;
    ...
    // FIXED: Check mounted after navigation returns
    if (!mounted) return;
  ...
  Future<void> _payWithCash(String orderId) async {
    ...
    // FIXED: Check mounted before navigating
    if (!mounted) return;
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(
...
class MenuItem {
  ...
  factory MenuItem.fromJson(Map<String, dynamic> json) {
    return MenuItem(
      ...
      name: json['name'] ?? '',
      description: json['description'] ?? '',
      // FIXED: was `(json['price'] ?? 0).toDouble()`, which throws/loses
      // the price silently if Supabase ever returns numeric as a string.
      // parseMoney() handles num, int, double, and numeric-string safely.
...
/// Local safe-parsing helper (mirrors menu_models.dart's parseMoney,
/// kept local here since this screen defines its own MenuItem/CartItem
/// rather than importing the shared model).
...
class CartItem {
  ...
  /// FIXED: this class had no totalPrice getter before, forcing every
  /// caller to recompute `price * quantity` manually — an easy place for
  /// one call site to be missed. Now there's one implementation.
  double get totalPrice => menuItem.price * quantity;
}

/// Sums every line in this screen's local cart map. Use this instead of
/// hand-rolling a fold() anywhere in this file.
double calculateLocalCartTotal(Map<String, CartItem> cart) {
  double total = 0.0;
...
int calculateLocalCartItemCount(Map<String, CartItem> cart) {
  int count = 0;
  for (final item in cart.values) {
    count += item.quantity;
  }
...
// Stub for Restaurant model (from browse_restaurants_screen.dart)