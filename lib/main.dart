...
final Color _subtitleColor = Colors.white.withOpacity(0.5);
final Color _hintColor = Colors.white.withOpacity(0.3);
final Color _fillColor = Colors.white.withOpacity(0.05);
...
class _RoleResolverState extends State<RoleResolver> {
  ...
  Future<void> _resolveRoute() async {
    ...
    if (user == null) {
      // No auth session — show login page (with guest option)
      if (mounted) {
...
class _CustomerHomeRouterState extends State<CustomerHomeRouter> {
  ...
  Future<void> _checkSession() async {
    ...
    if (userId != null && !isGuestMode) {
      // ✅ FIXED: Use 'group_sessions' (not 'table_sessions')
