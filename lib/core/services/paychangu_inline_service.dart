...
///
/// IMPORTANT: `payments` table (per current schema) has NO `tx_ref`,
/// `establishment_id`, `table_id`, `reference`, or `paid_at` columns.
/// - The unique transaction reference is stored in `idempotency_key`.
/// - `establishment_id` / `table_id` are stored inside `metadata` (jsonb),
///   since they're already derivable via `orders.establishment_id` / `orders.table_id`.
/// - Provider references are stored in `provider_payment_id`.
/// - Timestamps use `updated_at` / `webhook_received_at`, not `paid_at`.
class PayChanguInlineService {
  ...
  ///
  /// `payments` has no `tx_ref`, `establishment_id`, or `table_id` columns,
  /// so:
  /// - the transaction reference is stored in `idempotency_key` (unique)
  /// - `establishment_id` / `table_id` are stashed inside `metadata`
  ...
  ///
  /// Looks the row up by `idempotency_key` (there is no `tx_ref` column).
  Future<bool> updatePaymentStatus({
    ...
    try {
      ...
      // Both `reference` and `providerPaymentId` map to the same real column.
      if (providerPaymentId != null) {
        updates['provider_payment_id'] = providerPaymentId;
      } else if (reference != null) {
        updates['provider_payment_id'] = reference;
      }

      if (status == 'paid') {
        updates['webhook_received_at'] = DateTime.now().toIso8601String();
      }
  ...
  /// Get payment status by tx_ref (stored as `idempotency_key`).
  Future<Map<String, dynamic>?> getPaymentStatus(String txRef) async {
    try {
      final response = await _supabase
          .from('payments')
          .select('status, provider_payment_id, amount, updated_at, webhook_received_at')
          .eq('idempotency_key', txRef)
          .maybeSingle();
  ...
  /// Get full payment row by tx_ref (stored as `idempotency_key`).
