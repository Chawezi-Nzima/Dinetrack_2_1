...
class NotificationService extends ChangeNotifier {
  ...
  bool get isEnabled => _isSubscribed;
  ...
  /// Call this once after user login. Safe to call multiple times (idempotent).
  ...
  /// Enable notifications (subscribe to realtime). Alias for initialize().
  Future<void> enable() async => initialize();

  /// Disable notifications (unsubscribe from realtime but keep data).
  Future<void> disable() async {
    _unsubscribeFromRealtime();
    _isSubscribed = false;
    developer.log('NotificationService disabled', name: 'NotificationService');
  }
  ...
  void _subscribeToRealtime() {
    ...
    if (userId == null) {
      developer.log('Cannot subscribe: no user logged in', name: 'NotificationService');
      return;
    }
    ...
    try {
      _realtimeChannel = _supabase.client
        ...
        if (status == RealtimeSubscribeStatus.channelError) {
          developer.log('Realtime channel error: $error', name: 'NotificationService');
          _scheduleReconnect();
        } else if (status == RealtimeSubscribeStatus.closed) {
          developer.log('Realtime channel closed', name: 'NotificationService');
          _scheduleReconnect();
        } else if (status == RealtimeSubscribeStatus.subscribed) {
          developer.log('Realtime channel subscribed', name: 'NotificationService');
          _cancelReconnectTimer();
        }
      });
      ...
      developer.log('Realtime subscription error: $e\n$stackTrace', name: 'NotificationService');
      _scheduleReconnect();
    }
  }
  ...
  void _scheduleReconnect() {
    ...
    _reconnectTimer = Timer(const Duration(seconds: 5), () {
      ...
      developer.log('Attempting realtime reconnect...', name: 'NotificationService');
      _unsubscribeFromRealtime();
      _subscribeToRealtime();
    });
  }
  ...
  Future<void> markAsReadBatch(List<String> notificationIds) async {
    ...
    try {
      ...
      developer.log('Error batch marking as read: $e', name: 'NotificationService');
    }
  }
  ...
  /// Dismiss multiple notifications by ID.
  Future<void> dismissBatch(List<String> notificationIds) async {
    if (notificationIds.isEmpty) return;
    try {
      await _supabase.client
          .from('notifications')
      ...
      developer.log('Error batch dismissing: $e', name: 'NotificationService');
    }
  }
  ...
  /// Load user notification preferences from Supabase.
  /// Returns a map of notification type -> enabled status.
  Future<Map<NotificationType, bool>> loadPreferences() async {
    try {
      final userId = _auth.currentUserId;
      if (userId == null) return {};
      ...
      final prefs = <NotificationType, bool>{};
      final data = response as Map<String, dynamic>;
      for (final type in NotificationType.values) {
        prefs[type] = data[type.name] as bool? ?? true;
      }
      return prefs;
    } catch (e) {
      developer.log('Error loading preferences: $e', name: 'NotificationService');
      return {};
    }
  }

  /// Save user notification preferences to Supabase.
  Future<void> savePreferences(Map<NotificationType, bool> preferences) async {
    try {
      final userId = _auth.currentUserId;
      if (userId == null) return;
      ...
      developer.log('Error saving preferences: $e', name: 'NotificationService');
    }
  }
  ...
  static Future<void> _sendNotification({
    ...
    try {
      // Use the singleton instance to avoid creating new SupabaseService objects
      final supabase = _instance._supabase;
